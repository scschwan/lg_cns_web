...
public class DetailClusteringService {
    ...
    public Map<String, Object> getStatistics(String sessionId, int clusterId) {
        ...
        stats.put("mergedGroupCount", subMergeParents.size());
    ...
    public Map<String, Object> mergeClusters(String sessionId, int clusterId, List<Integer> clusterNumbers, String customMergeName) {
        ...
        // clusterId 스코프 검증 (★ null-safety)
        final int cid = clusterId;
        targets = targets.stream()
                .filter(c -> c.getClusterId() != null && c.getClusterId().equals(cid))
                .collect(Collectors.toList());
        ...
        for (ClusteringResult target : targets) {
            if (target.getClusterSubId() != null && target.getClusterSubId() > 0) {
        ...
        ClusteringResult merged = ClusteringResult.builder()
                ...
                .clusterSubId(newClusterNumber) // 자기 자신의 cluster_number
    ...
    public Map<String, Object> unmergeClusters(String sessionId, int clusterId, Integer mergedClusterNumber) {
        ...
        // cluster_sub_id로 자식 찾기 (부모 자신 제외)
    ...
    public Map<String, Object> mergeMergedClusters(String sessionId, int clusterId, List<Integer> mergedClusterNumbers, String customMergeName) {
        ...
        // cluster_sub_id로 자식 수집 (부모 자신 제외, ★ null-safety)
        List<ClusteringResult> allChildrenToMove = all.stream()
                .filter(c -> c.getClusterSubId() != null
                        && mergedClusterNumbers.contains(c.getClusterSubId())
    ...
    public Map<String, Object> addToMergedCluster(
        ...
        List<ClusteringResult> targets = clusteringResultRepository
                .findBySessionIdAndClusterNumberIn(sessionId, clusterNumbers);
        targets = targets.stream()
                .filter(c -> c.getClusterId() != null && c.getClusterId().equals(clusterId))
                .collect(Collectors.toList());
        ...
        Query childQuery = new Query(Criteria.where("session_id").is(sessionId)
                ...
                .and("cluster_number").ne(targetMergedClusterNumber));
        List<ClusteringResult> allChildren = mongoTemplate.find(childQuery, ClusteringResult.class);
    ...
    public Map<String, Object> addKeywordHierarchy(String projectId, Integer level, String parentId, String keyword) {
        if (level < 1 || level > 3) {
    ...
    /** 스코프 내 활성(미세부병합) 클러스터 - cluster_sub_id == -1인 것만 (★ null-safety) */
