...
|------|--------|----------------|----------|
| 1 | File Upload | uc_multiFileUpload | 세션 생성, excel 파일 s3 업로드 -> mongodb Insert |
| 2 | File Load | uc_fileLoad | mongodb 내에서 계정명 조건 별 데이터 추출 → raw_data Insert|
| 3 | Data Preprocessing | uc_Preprocessing | raw_data → process_data Insert|
| 4 | Data Analysis | uc_dataTransform | 집계, 그룹핑  -> clustering_results 에 1차 데이터 생성|
| 5 | **Clustering** | **uc_clustering** | **사용자 판단 기반 클러스터링**  -> clustering_results 내 cluster_id 컬럼 값 수정 ⭐ |
| 6 | **Export** | **uc_classification** | **Excel 내보내기 + 세션 완료** ⭐⭐⭐ |
| 7 | Sub Clustering | uc_detailClustering | 세부 클러스터링  -> clustering_results 내 cluster_sub_id 컬럼 값 수정 |
